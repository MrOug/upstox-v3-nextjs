...
export function UpstoxConsole() {
  ...
  const getStockList = async (): Promise<string[]> => {
    ...
    if (dataMode === 'topGainers') {
      log('Fetching Top 50 Gainers from API...');
      const gainers = await upstoxApi.getTopGainers(50);
      log(`✓ Retrieved ${gainers.length} gainers`);
      return gainers;
    }

    if (dataMode === 'topLosers') {
      log('Fetching Top 50 Losers from API...');
      const losers = await upstoxApi.getTopLosers(50);
      log(`✓ Retrieved ${losers.length} losers`);
      return losers;
    }
    ...
    <>
      {showChart && (
        <StockChart
          ...
          symbol={chartSearchQuery || chartInstrumentKey.split('|')[0]}
          companyName={COMPANY_FULL_NAMES[chartSearchQuery.toUpperCase()] || chartSearchQuery || 'Stock Chart'}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  ...
                  <select className="code-input" value={dataMode} onChange={(e) => setDataMode(e.target.value)}>
                    ...
                    <optgroup label="📈 Live Market Data">
                      <option value="topGainers">🔺 Top 50 Gainers</option>
                      <option value="topLosers">🔻 Top 50 Losers</option>
